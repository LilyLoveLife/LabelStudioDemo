...
const SiderMenu: React.FC<ISiderMenuProps> = ({ SiderProps: SiderProps }: ISiderMenuProps) => {
	...
	const userInfo = JSON.parse(localStorage.getItem('TEST_USERINFO') || '{}')
	...
	useEffect(() => {
		...
		const openKeysSet: Set<string> = new Set([...openKeys, ...getOpenKeys(location.pathname, MenuList)])
